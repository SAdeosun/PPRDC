...
def main():
    ...
    st.markdown ("This web application accepts an abstract text (or a batch of abstract texts in an Excel file) and classifies them into research domains including Clinical, Education, Social & Administrative, and Basic & Translational. It also returns the respective probabilities/scores for the abstract for the respective domains.") 
    ...
    if input_type == "Text":
        ...
        if uploaded_file:
            ...
            df_copy = df.copy()
            df_copy['Abstract_text_ignored'] = df_copy['Abstract'].apply(lambda x: '©' + x.split('©')[1] if '©' in x else None)
